...
class Maze:
    ...
    def extend_state(self, state, factor):
        ...
        for i in range(factor):
            new_states.extend([new_state[0] + i, new_state[1] + j] for j in range(factor))
        return new_states
...
def dyna_q(q_value, model, maze, dyna_params):
    ...
    while state not in maze.GOAL_STATES:
        ...
        for _ in range(dyna_params.planning_steps):
...
def figure_8_4():
    ...
    blocking_maze.old_obstacles = [[3, i] for i in range(8)]
